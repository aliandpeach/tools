...
public class DailyRegeneratingPattern implements TaskRecurrencePattern
{
    ...
    public DailyRegeneratingPattern(final int interval) {
        this.a = interval;
    }
    
    DailyRegeneratingPattern(final XMLStreamReader reader) throws XMLStreamException {
        this.a(reader);
    }
    ...
    public String toString() {
        String string = "<t:DailyRegeneration>";
        if (this.a > 0) {
            string = string + "<t:Interval>" + this.a + "</t:Interval>";
        }
        return string + "</t:DailyRegeneration>";
    }
